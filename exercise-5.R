...
words <- c("land","dry","cake","ramen","noodles","fish")
...
word.a.f <- substring(words, 1, 2) <= 'f'
a.f.words <- words[word.a.f]


# Create a vector `g.m.words' which are the elements in `words` that start with "g" through "m"
word.g.m <- substring(words, 1, 2) >= 'g' & substring(words, 1, 2) <= 'm'
g.m.words <- words[word.g.m]
